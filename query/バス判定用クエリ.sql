...
WITH table_car_or_bike AS (
  ...
  WHERE (pred_tpmode = "car" OR pred_tpmode = "bike")
    AND raw_element.latitude BETWEEN -90 AND 90
    AND raw_element.longitude BETWEEN -180 AND 180
),
...
dummy_probe_joined AS (
  ...
  FROM `ダミープローブデータ` bus
  JOIN `長野県メッシュデータ` mesh
    ON mesh.mt_mesh = CAST(convert_to_mesh(bus.lat, bus.lon) AS INT64)
  WHERE
    bus.lat IS NOT NULL
    AND bus.lon IS NOT NULL
    AND DATE(DATETIME(bus.date_time, "Asia/Tokyo")) BETWEEN "2024-01-01" AND "2024-01-14"
),
...
bus_flg_table AS (
  ...
  LEFT JOIN dummy_probe_joined AS bus
    ON convert_to_mesh_9(bus.lat, bus.lon) = convert_to_mesh_9(table_car_or_bike.latitude, table_car_or_bike.longitude)
    AND DATE(table_car_or_bike.sdk_detect_ptime, "Asia/Tokyo")
         = DATE(DATETIME(bus.date_time, "Asia/Tokyo"))
    AND table_car_or_bike.sdk_detect_ptime BETWEEN
      TIMESTAMP(DATETIME_SUB(DATETIME(bus.date_time, "Asia/Tokyo"), INTERVAL 30 SECOND))
      AND TIMESTAMP(DATETIME_ADD(DATETIME(bus.date_time, "Asia/Tokyo"), INTERVAL 29 SECOND))
    AND ST_DWITHIN(
      ST_GEOGPOINT(table_car_or_bike.longitude, table_car_or_bike.latitude),
...
bus_flg_table_v2 AS (
  ...
  LEFT JOIN `長野県バスルートデータ` AS poly
    ON ST_CONTAINS(ST_GEOGFROMTEXT(poly.polygon),
                   ST_GEOGPOINT(bft.longitude, bft.latitude))
),
bus_flg_cnt_2 AS (
  SELECT
    common_id,
...
group_car_table AS (
  ...
  LEFT JOIN table_car_v2 AS mv2
    ON convert_to_mesh_9(mv.latitude, mv.longitude)
       = convert_to_mesh_9(mv2.latitude, mv2.longitude)
    ...
    AND TIMESTAMP_DIFF(mv.sdk_detect_ptime, mv2.sdk_detect_ptime, SECOND)
        BETWEEN -29 AND 30
    AND ST_DWithin(
      ST_GeogPoint(mv.longitude, mv.latitude),
      ST_GeogPoint(mv2.longitude, mv2.latitude),
